...
fn fmt_type(this: &Type, f: &mut Formatter<'_>) -> Result {
    match this.0 {
        ...
        rustdoc_types::Type::FunctionPointer(fnp) => {
            ...
            write!(f, "{}fn{}", Header(&fnp.header), FnDecl(&fnp.decl, this.1))?;
...
fn fmt_function_header(this: &Header, f: &mut Formatter<'_>) -> Result {
    if this.0.const_ {
        write!(f, "const ")?;
    }
...
display_wrapper!(Header, fmt_function_header);

fn fmt_function_signature(this: &FnDecl, f: &mut Formatter<'_>) -> Result {
    write!(f, "(")?;
    enum Arg<'a> {
        Named(&'a str, Type<'a>),
        Dots,
    ...
    intersperse(
        ...
        this.0
            ...
            .chain(this.0.c_variadic.then_some(Arg::Dots)),
        |arg, f| match arg {
...
display_wrapper!(FnDecl, fmt_function_signature, bool);
...
fn fmt_where_predicate(this: &WherePredicate, f: &mut Formatter<'_>) -> Result {
    match &this.0 {
        ...
        rustdoc_types::WherePredicate::RegionPredicate { lifetime, bounds } => {
            ...
            intersperse(f, " + ", bounds, |bound, f| match bound {
                rustdoc_types::GenericBound::Outlives(s) => f.write_str(s),
                _ => unreachable!("lifetime with a trait bound: {this:?} {bound:?}"),
            })
        }
