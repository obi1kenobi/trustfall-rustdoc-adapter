...
use rustdoc_types::{
    Crate, Enum, Function, Impl, Item, Method, Path, Span, Struct, Trait, Type, Variant,
};
    ...
    fn typename(&self) -> &'static str {
        match self.kind {
            VertexKind::Item(item) => match &item.inner {
                ...
                rustdoc_types::ItemEnum::Variant(variant) => match variant {
                    Variant::Plain(..) => "PlainVariant",
...
impl<'a> Vertex<'a> {
    ...
    pub(super) fn as_method(&self) -> Option<&'a Method> {
        self.as_item().and_then(|item| match &item.inner {
