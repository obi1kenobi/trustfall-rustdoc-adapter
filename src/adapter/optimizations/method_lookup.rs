    ...
    if let Some(impl_owner_id) = find_impl_owner_id(impl_vertex) {
        match method_name {
            ...
            CandidateValue::Single(name) => {
                ...
                resolve_method_by_name(origin, impl_index, impl_owner_id, method_name)
            }
...
fn resolve_method_by_name<'a>(
    origin: Origin,
    impl_index: &'a HashMap<(&'a Id, &'a str), Vec<(&'a Item, &'a Item)>>,
    impl_owner_id: &Id,
    method_name: &str,
) -> VertexIterator<'a, Vertex<'a>> {
    if let Some(method_ids) = impl_index.get(&(impl_owner_id, method_name)) {
        Box::new(
            method_ids
                .clone()
                .into_iter()
                .map(move |(_, item)| origin.make_item_vertex(item)),
        )
    } else {
